"""HTTP service bridge (planned): expose falai tools as a `qh` app.

The plan: feed `falai.registry.list_tools()` callables to `qh.mk_app` so
...
def build_qh_app(*args, **kwargs):
    raise NotImplementedError(
        ...
        "`falai.registry.list_tools()` and pass them to `qh.mk_app`. See "
