...
``$FALAI_DATA_DIR/refresh/state.json`` (default ``~/.config/falai/refresh/``);
previous-version snapshots live alongside as ``<source>.prev`` so a successful
refresh can be diffed against the prior content.

For scheduled / remote runs, set ``FALAI_DATA_DIR`` to a path *inside* the
repo (e.g. ``misc/falai_state``) so journal entries and refresh state are
...
def _request(
    ...
    *,
    ...
    req = urllib.request.Request(
        url,
        method=method,
        headers={"User-Agent": "falai-refresh/0.1", **(extra_headers or {})},
    )
    try:
