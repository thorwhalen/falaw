"""Offline tests for falai.refresh.

We don't hit the network here. Network behavior is verified via the CLI
(`python -m falai refresh-llms`) and by the conditional GET we already
...
def _isolated_data_dir(tmp_path, monkeypatch):
    monkeypatch.setenv("FALAI_DATA_DIR", str(tmp_path))
    from falai.journal import _default_journal
...
def test_parse_index_dedups_and_filters(tmp_path):
    from falai.refresh import _parse_index
...
def test_conditional_headers_uses_saved_state():
    from falai.refresh import DocSource, _conditional_headers
...
def test_diff_against_previous_returns_unified_diff(tmp_path):
    from falai.refresh import DocSource, _snapshot_path, diff_against_previous
...
def test_state_roundtrip():
    from falai.refresh import _load_state, _save_state
...
def test_refresh_tools_registered():
    from falai.registry import list_tools
...
def test_fetch_if_changed_handles_304(monkeypatch, tmp_path):
    from falai import refresh as r
