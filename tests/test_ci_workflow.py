"""Guard the CI trigger shape that keeps a TS-only change from publishing Python (walkthru#23).

falaw ships two packages from one repo: the Python package at the root (published to PyPI by
...
Deliberately dependency-free: the reader below needs no YAML parser; CI installs no
...
def _on_block() -> list[str]:
    ...
    assert block, (
        ...
        "carry the path filter that keeps a TS-only push from publishing Python (walkthru#23)."
    )
    return block
...
def test_push_trigger_excludes_ts_only_changes():
    ...
    assert ignored, (
        ...
        "number for a change containing no Python (walkthru#23)."
    )
...
def test_pull_request_trigger_is_not_path_filtered():
    ...
    The fix for walkthru#23 gates the *publish*, which is reachable only from a push to the default
    ...
    assert not filters, (
        ...
        f"({filters}). walkthru#23 asks for a gate on the PUBLISH, not on the whole workflow: "
