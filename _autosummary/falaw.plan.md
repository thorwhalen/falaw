...
rate table by [`falaw.reprice_plan()`](falaw.md#falaw.reprice_plan); absent, that function reports the
...
([`falaw.reprice_plan()`](falaw.md#falaw.reprice_plan)) and audited after the fact.
...
once. ([`falaw.render_scene()`](falaw.md#falaw.render_scene) makes the same choice for the same reason.)
...
[`falaw.testing.fake_assets()`](falaw.testing.md#falaw.testing.fake_assets) is one line in a `conftest.py` — and the
...
[`falaw.reprice_plan()`](falaw.md#falaw.reprice_plan) reports as `"no_basis"` — never as a quote
...
the rest of a fan-out; it returns an [`ExecutionReport`](falaw.md#falaw.ExecutionReport) with one
...
fal has already billed — this returns an [`ExecutionReport`](falaw.md#falaw.ExecutionReport)
carrying one [`CallOutcome`](falaw.md#falaw.CallOutcome) per call: the successes with their
...
Public (no underscore) because it is now used from [`falaw.prune`](falaw.prune.md#module-falaw.prune) as
