...
falaw content-addresses every media result ([`falaw.content`](falaw.content.md#module-falaw.content)): a fal URL is
neither unique-per-content nor durable, so it cannot be an identity, and
[`falaw.execute_plan()`](falaw.md#falaw.execute_plan) therefore **reads the bytes** behind every result. A
...
[`falaw.plan.FETCH_BYTES_ENVVAR`](falaw.plan.md#falaw.plan.FETCH_BYTES_ENVVAR).
...
Install it with [`falaw.content.using_url_fetcher()`](falaw.content.md#falaw.content.using_url_fetcher) (the
...
Yield `url`’s bytes — the [`falaw.content.UrlFetcher`](falaw.content.md#falaw.content.UrlFetcher) to install.
