"""MCP bridge (planned): expose falaw tools as MCP tools.

The plan: walk `falaw.registry.list_tools()` and emit one MCP tool per
...
def build_mcp_server(*args, **kwargs):
    raise NotImplementedError(
        ...
        "`falaw.registry.list_tools()` --- the same source of truth used by "
